...
allowing to write useful notes for the specific date within the app, and then display the notes relevant to the current date of looking. <br>
...
**(LIMITATION)** If you decide to and update only the PDF files itself (if you would like to change something in the PDF), it's needed to reupload it only <br> 
...
**(FEATURE)** These implementation scripts do not require Internet, it works locally, on the RMPP. The implementation scripts do not send any data outside the RMPP. <br>
...
**(REQUIRED)** The `"ssh"` command available from the terminal. On MacOS it is available by default. For the Windows consult **Reference 2**. <br>
...
**NOTE**: It is crucial to use the version of Node.js itself `**22.X**`, that I've mentioned here, because there would be dependencies on it in scripts. <br>
