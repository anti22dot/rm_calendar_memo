...
What's most important that there is a loop, that runs indefinitely, which is `good` and `bad`. `Good`, because it updates the document fast. <br>
...
**(FEATURE)** These implementation scripts do not require Internet, they work locally on the RMPP. The implementation scripts do not send any data outside the RMPP. <br>
